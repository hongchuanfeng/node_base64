...
export default function TextBase64Tool({ title, description }: TextBase64ToolProps) {
  ...
  }, [input, encoding, addHistory, showToast]);
  ...
  const handleDecode = useCallback(() => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '解码失败，请检查输入是否为有效的Base64', 'error');
    }
  }, [input, encoding, addHistory, showToast]);
    ...
    <div className="tool-container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '1rem', alignItems: 'stretch' }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn btn-secondary" onClick={handleReset} title="清空">
                <RotateCcw size={16} />
              </button>
              <button className="btn btn-secondary" onClick={() => setShowHistory(!showHistory)} title="历史记录">
                <History size={16} />
              </button>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '1rem' }}>
          <button className="btn btn-secondary" onClick={handleSwap} title="交换">
            <ArrowUpDown size={20} />
          </button>
      ...
      <div className="card" style={{ marginTop: '1.5rem' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'center' }}>
          <div className="input-wrapper" style={{ minWidth: '150px' }}>
            <label className="input-label">编码格式</label>
            <select
              value={encoding}
          ...
          <div className="input-wrapper">
            <label className="input-label">操作模式</label>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button
          ...
          <div style={{ color: 'var(--text-tertiary)', fontSize: '0.8rem', display: 'flex', gap: '0.5rem' }}>
            <span className="shortcut-hint">Ctrl+Enter</span> 转换
            <span className="shortcut-hint">Ctrl+Shift+C</span> 复制
          </div>
        </div>
      </div>
      ...
      {showHistory && (
        <div className="card" style={{ marginTop: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ fontWeight: 600 }}>历史记录</h3>
          ...
          {history.length === 0 ? (
            <p style={{ color: 'var(--text-tertiary)' }}>暂无历史记录</p>
          ) : (
